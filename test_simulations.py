...
def compute_volume(catalog):
    return np.prod(np.max(catalog[:,:3], axis = 0) - np.min(catalog[:,:3], axis = 0))
...
def main():
    ...
    parser.add_option("--sigma_max_pos", type = "float", dest = "sigma_max_pos", help = "Maximum std for clusters", default = 2)
    parser.add_option("--sigma_max_vel", type = "float", dest = "sigma_max_vel", help = "Maximum std for clusters", default = 3)
    ...
    priors = sampler.run([catalog, 'mock_data', volume, None])
