...
def fill_matrix(dra, ddec, dplx, dpmra, dpmdec, dvrad, dradec, draplx, drapmra, drapmdec, ddecplx, ddecpmra, ddecpmdec, dplxpmra, dplxpmdec, dpmrapmdec):
...
def read_data(file):
    names = ['ra_error', 'dec_error', 'parallax_error', 'pmra_error','pmdec_error', 'radial_velocity_error', 'ra_dec_corr', 'ra_parallax_corr', 'ra_pmra_corr','ra_pmdec_corr', 'dec_parallax_corr', 'dec_pmra_corr', 'dec_pmdec_corr', 'parallax_pmra_corr', 'parallax_pmdec_corr', 'pmra_pmdec_corr']
    df = pd.read_csv(file)
    means = np.array([df['ra'],df['dec'],df['parallax'],df['pmra'],df['pmdec'],df['radial_velocity']]).T
    covs  = np.array([fill_matrix(*[df[name][i] for name in names]) for i in range(len(means))])
    
    print([df['ra'].max(),df['dec'].max(),df['parallax'].max(),df['pmra'].max(),df['pmdec'].max(),df['radial_velocity'].max()])
    print([df['ra'].min(),df['dec'].min(),df['parallax'].min(),df['pmra'].min(),df['pmdec'].min(),df['radial_velocity'].min()])
    return means, covs
    
def main():
    
    parser = op.OptionParser()
    parser.add_option("-i", "--input", type = "string", dest = "input_folder", help = ".csv from GAIA query")
